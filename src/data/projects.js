  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed?.groups?.length > 0 && parsed.groups.some((g) => g.projects?.some((p) => p.title))) {
        const savedDesignProjects = Array.isArray(parsed.designProjects) ? parsed.designProjects : null
        const onlyLegacySamples = savedDesignProjects?.length > 0
          && savedDesignProjects.every((project) => LEGACY_DESIGN_SAMPLE_IDS.has(project.id))
        return {
          ...defaultProjects,
          ...parsed,
          ...
          designProjects: savedDesignProjects && !onlyLegacySamples ? savedDesignProjects : defaultProjects.designProjects,
        }
      }
    }
  } catch {

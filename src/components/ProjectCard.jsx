...
function StoryTabs({ project }) {
  ...
  return (
    <div>
      {/* Tab bar */}
      <div className="flex flex-wrap items-center justify-center gap-0.5 pb-4 mb-5 border-b border-gray-700/40">
          ...
          {project.highlights && project.highlights.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2">
              {project.highlights.map((h, i) => (
                <div key={i} className="bg-accent/5 border border-accent/15 rounded-lg px-3 sm:px-3.5 py-2 sm:py-2.5 text-center min-w-[70px] sm:min-w-[84px]">
                  <div className="t-num text-sm sm:text-base md:text-lg font-bold text-accent leading-tight">{h.value}</div>
